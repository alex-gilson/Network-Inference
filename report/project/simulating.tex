...
\section{Differences between the real and simulated biological neural network}\label{sec:differences_simulation_real}
...
\begin{table}[]
	...
	\begin{tabular}{|c|c|c|c|c|c|}
		\hline
		Number of neurons & Total & Mean   & Median & STD    & Freq spikes/neuron \\ \hline
		98                & 636878           & 6498.2 & 3723.2 & 7849.2 & 1.2                \\ \hline
	\end{tabular}
	\caption{Spike characteristics of a 98 neuron network from the somatosensory cortex of a mouse}
	\label{tab:spike_characteristics}
\end{table}

As it can be observed in table \ref{tab:spike_characteristics}, one of the most obvious changes with respect to the previous simulations is the total number of spikes. This number is significantly larger than before due to the fact that the observation time is larger. In \cite{alexandru2018estimating} the total observation time was equal to the defined length of stimulation multiplied by the number of neurons. Since the stimulation period was found to be optimal for 4 seconds, then for a network of 98 neurons, this would result in a total of 392 seconds of recordings. 
...
Previous work \cite{alexandru2018estimating} simulated a network whose input was a DC voltage that would stimulate each of the neurons one by one. 
