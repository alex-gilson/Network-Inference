...
\begin{tikzpicture}[auto, thick, node distance=4cm, >=triangle 45]
    \node at (0,0) [block] (a) {\(\textbf{t}^{c}\)};
    \node [block, right of=a, node distance=4cm] (b) {\(\log S(t_{i}| t_{j};\alpha_{j,i}), H(t_{i}| t_{j};\alpha_{j,i})\)};
		\node [coordinate] (c) [right of=b, node distance=2cm] {};
		\node [coordinate] (d) [above of=c, node distance=2cm] {};
		\node [block, right of=d, node distance=2cm] (e) {\(\Pi\)};
...
\subsection{Recursive quadratic approximation algorithm}
