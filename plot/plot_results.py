...
for n, size in enumerate(network_sizes):
    ...
    plt.figure()
    plt.plot(simulation_times, np.mean(maes[n], axis=1), label='MAE', color='#5a3aa9')
    plt.plot(simulation_times, np.mean(accuracies[n], axis=1), label='accuracy', color='#00a0ff')
    plt.plot(simulation_times, np.mean(precisions[n], axis=1), label='precision', color='#8b9dc3')
    plt.plot(simulation_times, np.mean(recalls[n], axis=1), label='recall', color='#eb104e')
    plt.legend()
    plt.grid()
...
#
#     plt.figure()
#     plt.plot(simulation_times, np.mean(maes[:,n,:], axis=0), label='MAE', color='#5a3aa9')
#     plt.plot(simulation_times, np.mean(accuracies[:,n,:], axis=0), label='accuracy', color='#00a0ff')
#     plt.plot(simulation_times, np.mean(precisions[:,n,:], axis=0), label='precision',color='#8b9dc3')
#     plt.plot(simulation_times, np.mean(recalls[:,n,:], axis=0), label='recall', color='#eb104e')
#     plt.legend()
#     plt.grid()
#     plt.xlabel('length of simulation (s)')
