...
A new dataset has been made available with real neuron spike information from a mouse somatosensory cortex \cite{ito2016spontaneous, ito2014large, litke2004does}. This data contains several recordings lasting one hour with an average spiking frequency of 2.1Hz. The minimum, maximum and average number of neurons in each of the recordings is 98, 594 and 309, respectively. An ultimate test to the algorithm would be to infer the connectivity of such a network. However, several problems arise:

\begin{enumerate}
\item The minimum number of neurons in the dataset is 98 and the algorithm has only been employed for networks of a maximum of 50 nodes. This is due to the fact that it is computationally expensive to compute larger networks. In order to apply the algorithm to this dataset, the computation time must be significantly reduced.
\item The input stimulus model of the dataset and that of the simulation do not match. The data contains observations from spikes without stimulation (i.e spontaneous activity) and the algorithm is designed for a DC input stimulus. The shape of the likelihood function could be different and it would, therefore, be necessary to estimate it again for the new model.

\item One of the reasons for using a DC input stimulus was to have a systematic way of selecting the beginning of a cascade. Every time the selected stimulated neuron spiked, a new cascade originated \cite{pranav_report}. A new method must be devised for cascade generation in the dataset by which NetRate requirements are met and the cascades contain high quality data. 

\item Since there is no ground truth for the connectivity of the network in the dataset, there is no obvious way of evaluating the performance of the algorithm. Some sort of data splitting is necessary for determining how good the algorithm is. 

\end{enumerate}

In this project, a solution to each of these issues will be attempted. The dataset also contains information about the location of each of the neurons. It could be interested to evaluate if there is any correlation between connectivity and proximity.

\subsection{Increase the similarity between simulated and real neural networks}
...
\item Introduce inhibitory neurons into the network. In the mammalian brain, there is a ratio of 4 to 1 excitatory to inhibitory neurons \cite{izhikevich2003simple}. The simulated network in \cite{alexandru2018estimating} only includes excitatory neurons and, therefore, it is not an accurate representation of the biological network.

\item The network can also be made more reallistic by adding different types of excitatory and inhibitory neurons. The only ones considered in \cite{alexandru2018estimating} were the regular spiking (RS) neurons. However, as discussed in section (\ref{sec:izhikevich_neuron_model}), there are many different types. 

\end{enumerate}

All these changes to the network would require a new method for infering the weights of the adjancency matrix and this could prove to be challenging. For this reason, the feasibility of this option will only be considered when other, more important milestones, are completed.

\subsection{Estimated timeline of the project}

The date of submission of the report is in mid-June. This gives the project a time frame of almost 5 months. This is sufficient time for a succesful completion of the project. However, a timeline, like the one seen in table \ref{tab:project_timeline}, is required in order not to fall behind.
\begin{table}[!h]
    \centering
    \begin{tabular}{|c|c|}
		\hline
		Month & Milestones \\
		\hline
		 & Evaluate the increase in speed due to parallelism. \\
		 February & Create a function that relates the computation time\\
		 & of the algorithm to the number of nodes. \\
		\hline
		& See if there are more adequate datasets.  \\
		March & If not, create an algorithm for cascade selection \\
		& under the new input stimulus\\
		\hline
	  & Simulate the network under the new conditions and \\
		April & assess the performance. Investigate if the shape of the \\
		& likelihood function needs to be changed. \\
		\hline
		& Apply the algorithm to the dataset and draw the \\
		May & inferred network. Find a way of evaluating the  \\
		& performance of the algorithm.\\
		\hline
		June & Increase the complexity or of the simulated network  \\
		\hline
		\end{tabular}
		\caption{Timeline of the project from February to June}
		\label{tab:project_timeline}
\end{table}

\section{Evaluation plan}

In order to test the results of this project some performance metrics can be used related to the deliverables set in section \ref{sec:implementation_plan}. These results will be compared to those obtained in \cite{alexandru2018estimating}. However, special care must be taken since they could end up being compared under different test conditions (e.g network complexity is different).

\begin{enumerate}
\item Determining the speed of the algorithm with respect to the number of nodes will provide information about how much faster the algorithm has become by being parallelized. If the algorithm is proportionally speed up with respect to the number of processors, then this deliverable of the project will have been a success.
\item Improving the accuracy of the algorithm or keeping it fixed while testing on more complex networks will be an indicator of success for this section of the project.
\end{enumerate}

A major project deliverable to apply the algorithm on a real neural network dataset. Some of the issues related to this deliverable were explained on section \ref{sec:implementation_plan}. If these problems are solved, the project will have been a complete success.
...
Two software packages are required, namely, CVX \cite{cvx,gb08} and the Brian Simulator \cite{10.3389/neuro.01.026.2009}. Both of them are open source for the scope of this project. The only requirement is to cite the authors.
